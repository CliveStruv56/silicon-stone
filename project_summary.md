...
| Commit | Description |
|--------|-------------|
| _(follow-ups)_ | **Banned-word heading rename + discreet `/eu-exposure` entry point.** (1) Renamed the visible `<h2>` on article pages (`analysis/[slug]/page.tsx`) from "Actionable Insights" (banned phrase) to **"What to do next"**; the Sanity `actionableInsights` field name + Studio label are unchanged (editor-facing). (2) Added a discreet **"EU exposure (US firms)" → `/eu-exposure`** link to the footer Engage column (`Footer.tsx`) — site-wide but unobtrusive; main funnel pages' narrative untouched. (3) Logged a reminder (Known Issues + auto-memory) to segment Kit leads on the `interest` tag once ConvertKit is configured. |
