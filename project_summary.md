...
| **Annotations** | `scripts/annotate-glossary.ts` (`npm run glossary:annotate`) — idempotent, re-runnable, splits the target span and inserts a `glossaryTerm` markDef on the **first whole-word mention** in a normal paragraph (headings skipped; per-term skip if already marked). **7 articles, 60 annotations** (all resolve): eu-ai-act-compliance-chasm (6), korean-memory-fab-capacity-squeeze (18), iran-conflict-european-technology-supply-crisis (10), semiconductor-testing-bottleneck (9), helium-scarcity-semiconductor-production (8), atlantic-fault-lines-us-tech-policy (6), us-national-ai-policy-acceleration (3). Extend the `PLAN` map to annotate more articles. |
